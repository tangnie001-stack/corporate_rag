...
**数据源**：`RequestContext` 已携带 `agent` / `agent_display_name` / `loaded_skills` / `skill_action` / `kb_id` / `kb_domain` / `deep_thinking` / `has_skills`，无需新增取数。用户身份由认证中间件的 `current_user_id` 提供。
...
- **fork 子代理的工具调用**、**工具内部子步骤**（embed / 向量检索 / rerank）—— 不在主事件流内，本期不覆盖。
...
**决策**：只用 `Langfuse.span(trace_id=…)`，**永不调用 `client.trace()`**。这同时把「命令式路径是否会改写根 trace」这一整类风险消除。
...
- `_round: {…: span}` 或「当前 tools 父 span」：`on_chain_start/end(name=="tools")` 开合。
...
- 按 `metadata` 过滤子图 / 子代理事件（子图有自己的 `checkpoint_ns` 与 step 计数）。
...
### D6 trace 富化：字段与 tags 口径

`update_current_trace(input=…, session_id=…, user_id=…, tags=…, metadata=…)`：

- `user_id` = `current_user_id.get()`（未登录时是 cookie 里的匿名 uuid，仍可用于「同一访客多轮」聚合）。
...
- **0 视为未配置**：CLI 跳过 seed —— 避免在 Langfuse 里留下一个会被误读成「免费」的 ¥0 模型定义。填了真实单价再 seed。
- 落地走 repo 内**幂等** CLI（`langfuse.api.models` 的 `list` + `create`），dev / prod 各跑一次；比 UI 手配可复现，比 `cost_details` 保留模型定义页与成本分析能力。
...
- **子图 / 子代理事件混入** → 按 `metadata` 的 `langgraph_node` / `checkpoint_ns` 过滤；fork 子代理事件本就走 `clarify_channel`（`scope != main`）。
- **验收只能在 dev 真跑**（Langfuse 服务端行为无法用单测替代）→ E2E 清单：一轮含 ≥3 迭代、≥2 次工具调用、工具轮 `output` 非空、每轮 `tools` 父 span + 子 span、`user_id`/`tags`/`metadata` 落库、`totalCost > 0`（seed 定价后）、取消路径无悬空 span。
- **配置字段默认 0 会让「成本仍不可见」看起来像没做完** → 由 D7 的口径 + cookbook 操作步骤覆盖；DoD 里把「seed 后 `totalCost > 0`」列为可选项，不阻塞主体验收。

## Migration Plan

1. 代码落地后在 `LANGFUSE_ENABLE=false` 下回归一轮（零产出、行为不变）。
2. dev 侧开启后跑 E2E 清单。
3. dev 侧填单价并跑一次 seed CLI，确认 `totalCost > 0`。
