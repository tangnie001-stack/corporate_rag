...
- 成本空：`models` 表只有 OpenAI 系列定价，无 `qwen3.8-flash`；dev 库 8 条 generation 的 `calculated_total_cost` 全为 NULL。
...
| `StatefulClient.span()` 只入队 `span-create`，**不**重发 `trace-create` | 从「trace 客户端」挂 span 不会动 trace |
| 命令式 `client.trace(id=…)` 只传 id 时，`name`/`input`/`output`/`metadata`/`tags` **不被清空**（服务端 `?? undefined` 过滤），但 **`timestamp` 被无条件覆盖**。**注意：`update_current_trace(...)` 内部也走这条路**（`langfuse_decorator.py` 的 `update_current_trace` 会调 `client_instance.trace(id=…)`），所以**每一次 generation 都在刷新 trace 的 `timestamp`** —— 这是**既有行为**，本变更不改变它 | 见 D3：工具 span 路径不去碰 trace 行即可，**不要把"timestamp 漂移已被根除"写成收益** |
...
- ⚠️ **不得**写成「trace 的 `timestamp` 漂移已被根除」：`update_current_trace(...)`（既有 generation 路径，每轮都调）**内部就是走 `client_instance.trace(id=…)`**，因此每轮 generation 仍在刷新 trace 的 `timestamp`。那是**既有行为**，本变更不改变、也不声称修好。
...
- `user_id`：**在请求内捕获后显式传入**，不依赖 contextvars 继承。实测 `asyncio.create_task` **会**复制上下文（`current_user_id.get()` 在任务内能取到值），但 `src/api/chat.py:42-43` 的注释断言「任务与请求不共享 context」，与该用法**自相矛盾** —— 一旦有人按注释去「修正」或调整中间件顺序，`user_id` 会**静默变空**。做法与 `trace_id` 对齐：捕获 + 显式传参，并顺带订正那条误注释。
  - ⚠️ **空串必须转成 `None`**：`update_current_trace` 的字段过滤是 `v is not None`（**不过滤空串**），直接传 `""` 会把空字符串写进 trace。落点写 `user_id or None`。
...
- **货币与量纲（易错，必须写死）**：`input_price` / `output_price` 的单位是 **USD / 单个 token**（不是每 1M）。例：$3 per 1M tokens 要填 `0.000003`。填成「每 1M」会让成本差 **1e6 倍**，而"成本 > 0"的闸门照样通过 —— 必须在配置注释里给出该换算例子。
- **模型名不硬编码**：`model_name` 取 `settings.LLM_MODEL`（当前 `.env` 覆盖为 `qwen3.8-flash`，但 `settings.py` 的内置默认是另一个值）；`match_pattern` 由它构造为锚定正则 —— `"(?i)^" + re.escape(model_name) + "$"`，避免手写漏转义。
- **pattern 写错的真实后果是「误配」而不是「失配」**：Postgres 的匹配走 `event.model ~ match_pattern`（正则、**子串匹配**）。未锚定/未转义的 pattern **仍然会匹配**，但会**把单价套到相近命名的其他模型上**（例如带版本后缀或形近写法的模型）。所以锚定 + `re.escape` 是为了**防止误配**。
...
- **seed CLI 的幂等判据按 `model_name` 已存在即跳过**：只比 `model_name`（Langfuse 侧同名即视为同一定义），不比 `match_pattern` —— 判据简单可预期，避免「改了 pattern 就多出一份定义」这类隐蔽行为。`model_name` / `match_pattern` 取值见 D7。
...
- **模型名硬编码会随配置漂移而静默失配** → `model_name` 取 `settings.LLM_MODEL`；两者不一致时告警/拒绝 seed（D7）。
...
- **成本可能在无 worker 时不算**（容器内没有 langfuse-worker 进程，摄入路径含 `event.model ~ match_pattern`，判断为 in-process，但未实跑）→ 先做一次 seed + 一轮对话，确认 `calculated_total_cost` 真落库，再依赖「成本 > 0」闸门；否则该闸门会**卡住**而不是静默通过。

## Migration Plan

1. 代码落地后在 `LANGFUSE_ENABLE=false` 下回归一轮（零产出、行为不变）。
2. dev 侧开启后跑 E2E 必过清单。
3. dev 侧填单价（USD / 单 token 口径）并跑一次 seed CLI，确认 `calculated_total_cost` 真落库且非零。
