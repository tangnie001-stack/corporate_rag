...
| 2026-09-19 | change §9 之后的 dev 环境状态 | ① `docker-compose.override.yml` 把 `./src` 与 `./tests` **挂进容器**（实测），所以 **Task 2–8 的代码改动 `docker compose restart app` 即生效、无需 rebuild**；只有 Task 9 改了依赖才需要 `build`，且 `CLAUDE.md` 推荐 `--no-cache`。② P1 后 PG 是全新库，界面会"变空"，但**登录会自动注册**（`src/api/auth.py:38-41`），**不需要 seed 账号**。③ `TEST_ACCOUNT`/`TEST_PASSWORD` **全仓 `.py` 零引用**（只在 `docs/agents/cookbook.md:92` 作手工 API 调试凭据）—— 我此前写的"测试依赖它"是错的 | 实施计划增「运行中的 dev 栈（改代码怎么生效）」与「P1 结束时你会看到什么」两节；Task 9 的 `build app` 改 `build --no-cache app`；4 处 `TEST_ACCOUNT` 错述已更正 | 是（已并入 design Risks） |
