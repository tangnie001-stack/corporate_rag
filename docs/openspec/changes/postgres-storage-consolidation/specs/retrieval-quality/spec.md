...
**dense 路** SHALL 以**迁移等价性**验收：固定查询集（≥20 条，覆盖单知识库的中文 / 数值 / 时间三类），在同一份语料与同一批查询下比对替换前后的 top-k，**重合率 SHALL ≥ 0.9**。验收范围 SHALL 只覆盖单知识库路径（全局检索路径随 `similarity_search_all` 一并删除）。
