...
- [ ] 3.3 去重键：**`(doc_id, hash(parent_content))`**；每键保留**代表分最高**的一条 —— **代表分由调用方显式传入**（rerank 路径传 `relevance_score`；精排超时降级路径传 `1 - distance`），函数内部**不得自取** `distance`，否则降级路径的"保留最高分"会退化为"保留融合首条"；`parent_content` 缺失时按该 chunk 自身保留（不参与折叠）。⚠ 必须把 `doc_id` 纳入键（跨文档样板文本会误折叠）；不要用 `heading_path`（实测 `(doc_id, heading_path)` 在 11/51 组里对应多个父块）；不要用全文做 dict key
