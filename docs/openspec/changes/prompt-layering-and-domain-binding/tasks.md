...
- [ ] 1.0 **把 `src/config/prompts.py` 升级为包 `src/config/prompts/`**（⚠ 前置，必须先做）：`git mv src/config/prompts.py src/config/prompts/__init__.py`；`__init__.py` 保留原公开符号的 re-export，使既有 `from src.config.prompts import X` 路径不变。⚠ **绝不能让 `prompts.py` 与 `prompts/` 并存** —— 同名包会遮蔽同名模块，8+ 个调用点会全数失效（见 spec `<prompt-carrier>`「YAML 载体与目录结构」）
- [ ] 1.1 在 `src/config/prompts/templates/` 下建模板文件。⚠ **迁移范围是 19 个常量中的 12 条，不是全部**（见 `specs/prompt-carrier/spec.md` 的「迁移范围」）：**迁入 YAML** 的是 ① 5 条段模板来源（`FINANCIAL_SYSTEM_PROMPT` 拆分重组、`KB_BOUND_RETRIEVAL_DISCIPLINE` 并入 `sources`、`KB_UNBOUND_SYSTEM_PROMPT`、`INLINE_CITATION_INSTRUCTION`、`DELEGATE_GUIDANCE_SECTION`）② 用户请求模板 `USER_PROMPT_TEMPLATE` ③ 6 条离线任务模板（classifier / rewrite / entity 各 2）。**保留为 Python 常量的 7 条**：`VERIFY_*` 4 + `FORK_*` 3 —— 它们是**行为键**（marker 是查重的键、`FORK_EXECUTION_CONTRACT` 是执行契约），模板化等于把行为键交给文案编辑者。⚠ **P0 阶段每条原常量对应一个模板、正文逐字搬入**（`content: |` 块标量），段拆分重组属 P1；`FINANCIAL_SYSTEM_PROMPT` 只搬**拼接前的基础段**，`DELEGATE_GUIDANCE_SECTION` 单独一条，避免同一段文本在 YAML 里存两份。字段按 `kind` / `section` / `domain` 声明（见 1.11）
