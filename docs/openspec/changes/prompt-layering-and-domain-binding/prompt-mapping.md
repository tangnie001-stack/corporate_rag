...
| 检索阶梯首条：`显式选定来源时，知识库检索是补充而非前置；未选定来源且问题属实质性提问时，先检索再作答…` | `grounding_prompt.go:46-51`（即本节依据表「显式选定来源时 KB 检索是补充而非前置」一行） | **有意差异（勘误 4，T5b 核对）**：落盘未含「显式选定来源时，知识库检索是补充而非前置」半句，并把"先检索再作答"写成无条件的 `实质性提问先调用 retrieve_kb 检索再作答`（保留本文件标 **S** 的"先调用 `retrieve_kb`"动作）。本任务判定为"核对 + 登记"而非改模板，理由：① 不影响本变更 spec 的 ladder 要求 —— `specs/prompt-composition/spec.md:133-134` 的条件判据表只列"先检索再作答 / 不预判范围 / 含核心实体才算相关"与"换问法重检 / 第二次 `top_k=10`"；② 遵循"最小改动"，不在本任务引入新的"前置 vs 补充"前置条件（其语义与 `sources-general`「遵守用户当前的来源限制与明确选择」、`runtime_contract`「本轮的来源选择…」相邻），留待对端 change `retrieval-fetch-and-dedup` 结论落地时一并复核 |
