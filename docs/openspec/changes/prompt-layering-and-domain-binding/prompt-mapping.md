...
| 来源限制条：`…已接入来源**；**用户选定…` | — | **有意差异（勘误 1，T5a 落盘）**：本文件「更改后」正文此处为句号断句（`已接入来源。用户选定…`），落盘改为分号，把"来源选择"与"用户选定不排除互补来源"归入同一条 bullet |
| 证据不足条：`…但不足以回答问**时**：…` | — | **有意差异（勘误 2，T5a 落盘）**：本文件「更改后」正文无「时」（`但不足以回答问题：`），落盘加「时」，与同段其余条件句"…时："句式一致 |
