...
|---|---|---|---|
| ① | 测试重定先完成；"pytest 全绿"须在全局关停 tracing 前提下取得 | **前置达成**：`tests/conftest.py` import 期关停 + session 级 fixture 兜底；`tests/config/test_langfuse_disabled.py` **3 passed**；`tests/test_tracing_order.py` 以列表相等断言 `configure → flush` 的调用序（覆盖 lifespan 与 CLI 两条路径）。**"全绿"未达成**：收尾实测 `pytest tests/ -q` → **33 failed / 1117 passed / 11 skipped / 31 warnings / 65 errors（耗时 18:49）**；failed=33 与改动前基线一致（中间态曾为 34 —— 那条本 change 自引入的回归已修掉）；passed 1107→1117（**+10**，为新增 tracing / purge 用例）；33 条失败全为**存量环境类**（`tests/infra/db/*` 的 `socket.gaierror`、`tests/parsers/*` 的 `FileNotFoundError`），非本变更引入；非 DB / 非 parser 子集全绿 | **部分**：前置条件 ✅；"全绿"受**存量环境失败**阻塞 |
