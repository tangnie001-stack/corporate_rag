...
- [ ] 5.11 测试：前缀路由 / **前缀清洗（当前轮与历史都不含 `/name`，落库仍为原文）** / **`/xxx` inline 单轮与 fork 直出（断言主 agent 0 LLM 轮 + citations 非空 + 无 `INVALID_CITATION`）** / 注入后持续生效 / 双轴过滤 / **绑定四态（首轮绑定、沿用、忽略+warn、未注册降级）** / 老会话 `bind-if-empty` 可绑定 / `agent_used` 流事件（**发出**判别用例：`tests/services/test_agent_service.py::test_run_generation_emits_agent_used_event` 断言缓冲载荷为 `[{"type":"agent_used","agent":"finance-expert"}]` 且为首个事件；`tests/services/test_dual_stream.py::test_stream_chat_aclose_does_not_abort_generation` 断言 `stream_chat` 首产出为 `SSEAgentUsedEvent`；序列化契约：`tests/utils/test_sse_roundtrip.py::test_sse_roundtrip`）/ **未选 agent 时 system 段快照逐字一致** / 两接口信封结构 + 服务端过滤 + 失败降级 / **非法名称跳过** / `sessions/messages` 契约不变（`data` 仍为数组）——**已由 Plan 3 完成**（各断言落点逐条核对见本 change 收口报告 task-10-report.md）
...
- [ ] 7.2 `docs/agents/glossary.md`：新增「智能体预设」「会话级 vs 消息级」术语——**已满足（此前已登记）**（两条在 Plan 3 之前已登记于 glossary.md:78-79「智能体预设与调用控制」表，收口核对无需新增）
