...
- **新建对话时选择智能体**：新对话页在知识库选择器**右侧**新增智能体选择器（默认项文案「默认」）；**首次绑定即固化**，之后以绑定值为准（传入不一致**不报错、不阻断**，只记 warning 报警，见 design D1）。
- **智能体作为会话主 agent**：选定 agent preset 后，其正文作为本会话主 agent 的 system prompt（注入点 `src/rag/prompt.py`），会话内持续生效。
- **`/xxx` 显式调用 skill**（消息级触发）：用户可在任意一条消息用 `/name` 前缀加载 skill，**不受智能体选择影响**（skill 与 agent 解耦）；输入区另提供**技能选择器**（深度思考旁下拉）作为按钮式入口，选中即插入 `/name `，与 `/` 补全共用同一候选与同一后端解析。执行形态：`inline` 由主 agent 单轮作答；`fork` **子代理直出（主 agent 0 次 LLM 轮）**，结果交编排层 `verify` 校验（0 LLM），不通过最多重跑 1 次。未知且形如命令的 `/name` → 提示"不存在 + 可用列表"（不静默）；不构成命令形态 → 按普通文本。
...
- **system prompt 三层组装**：① 人设层（未选 agent = `get_base_system_prompt()` 原文；选了 = preset 正文）+ ② 环境约束层（引用要求 / 委派引导 / KB 检索纪律 / 禁止检索 / 日期，**系统强制叠加，不可被 preset 覆盖**，本地常量不经远端）+ ③ 运行时层（inline skill / verify 指引）。`PromptManager` 需拆出 `get_base_system_prompt()`，追加段顺序与现状一致以保证行为不漂移。
- **子代理不直接交互 + 确认门**：子代理不持有 `ask_user`；需要确认时由**编排层确认门**（规则判断）走 ask_user，用户答后**重跑子代理**（上限 1 次）。
- **能力清单接口**：`GET /api/skills`（registry 的 `user_visible()` 派生）、`GET /api/agents`（registry 全部可加载预设，含 `display_name`），供前端选择器与 `/` 补全；走统一响应信封；**不设 catalog 文件**（避免与 frontmatter 双维护；新增预设只需放一个文件）；skills 服务端过滤 `user-invocable: false`；默认项「默认」由前端合成（`value=""`）。
...
- `agent-preset`: 智能体预设的内容契约（`agents/<name>.md` 字段）、加载与注册表、新建对话时的会话级选择与首次绑定规则、作为会话主 agent 人设注入
...
- `capability-catalog`: 两个只读接口（`GET /api/skills` / `GET /api/agents`）由 registry 派生（无清单文件）、统一响应信封、服务端候选过滤、默认项由前端合成、展示名归属内容文件
...
- `agent-service`: 流式请求契约新增 `agent`（会话智能体）；会话智能体**首次绑定即固化**（bind-once，不一致忽略并 warning，不阻断）；会话装配按 agent preset 注入人设；`/xxx` 路由与执行形态（inline 单轮 / fork 直出 + 编排层 verify）
...
- **新增内容目录**：`agents/`（智能体预设内容库，业务侧管理，compose volume 挂载）；**不新增清单文件**（列表由 registry 派生）
...
- **修改代码**：`src/agents/skills/`（models/loader/registry/executor/delegate_task——删 `thinking`/`max-iterations`、双轴、`agent_prompt`→`fork_body`、fork 改 `create_agent` + 执行者与工具 + `delegate_id` 分槽）、`src/agents/tools/registry.py`（ToolEntry.readonly）、`src/rag/prompt.py`（**三层组装**）、`src/infra/llm/prompt_manager.py`（拆出 `get_base_system_prompt`）、`src/config/prompts.py`（环境约束层常量拆分）、`src/services/agent_service.py`（装配 presets + `agent` 绑定/沿用 + 前缀路由与执行形态）、`src/api/model/request.py` + `src/api/chat.py`（`agent` 字段 + 绑定校验在流开始前）、`src/chat/manager.py` + `src/chat/persistence.py` + `src/infra/db/mysql_db/chat_repo.py`（会话存/取 `agent` + `bind-if-empty`）、`src/api/model/response.py` + `src/api/sessions.py`（**`sessions/list` 返回 `agent`**；`sessions/messages` 契约不变）、`src/infra/llm/request_context.py`（`delegate_id` 分槽）、`src/services/capability_service.py` + `src/api/capabilities.py`（registry 派生清单接口）
- **数据库**：`sessions` 表新增 `agent` 列（**手工 SQL 迁移** `scripts/migrations/`，default `''`；回滚可保留该列，空值即沿用旧行为。不新增 alembic revision——该链已分叉，见 Risks）
- **前端**：`deploy/nginx/html/chat.html`（新建对话页智能体选择器 = 知识库右侧 + 技能选择器 = 深度思考右侧 + `/` 命令补全 + 请求体 `agent` + `agent_used` 纠正标识）；实现走 `frontend-design` skill，按 `docs/design/pages/chat-agent-skill-selector-2026-09-11.md` 规格与 `chat-agent-skill-selector-mockup-2026-09-11.html` 落地，改完用 `playwright-cli` 验证并同步 `MASTER.md`
- **契约/文档**：`docs/agents/api_contract.md`（`agent` 字段与沿用语义 + `/xxx` 语义 + `sessions/*` 新增字段 + `agent_used`）、`docs/agents/glossary.md`（agent preset / 会话级 vs 消息级）、`docs/agents/code-map.md`（`agents/` 目录）、`docs/agents/reference-projects.md`（agency-agents 作为预设来源）、`docs/agents/logging-rules.md` + `src/core/log_events.py`（新增事件登记）、`docs/agents/defensive-patterns.md`（并发：活跃状态按 id 分槽）
...
- **部署**：`agents/` 需 volume 挂载（同 `skills/`，dev/prod 两个 compose 都要）；数据库走**手工 SQL 迁移**加 `sessions.agent` 列（`scripts/migrations/`，与既有实践一致）；**无新增依赖**（`langchain.agents.create_agent` 在已装 `langchain==1.3.11` 中即存在，本 change 不升级依赖）
