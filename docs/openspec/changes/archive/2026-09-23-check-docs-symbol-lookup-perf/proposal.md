...
- **有意保留两处现状边界行为**并登记为独立要件（本次不修）：(a) `#` 出现在字符串字面量内也被当注释起点（失败方向单一，只产生 `warn` 噪声，**触发面与改前相同**）；(b) `UnicodeDecodeError` 不被捕获（单个非 UTF-8 的 `.py` 可挡死全员提交；**触发面因快照必读全树而较改前扩大** —— 故本次"改前 / 改后 0 差异"证明的适用输入域是"`src/` 下全为 UTF-8"）。保留是为不动代码、维持等价性证明的纯净；取舍与非对称严重度见 `design.md` D6。
