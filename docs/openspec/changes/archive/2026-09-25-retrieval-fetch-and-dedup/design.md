...
- **父块正文会被重复渲染**：`retrieval.py:168-172` 用 `parent_content` 覆盖 chunk 正文，`context.py:60` 渲染的正是它。实测父块正文约 1998 字符；平均每个父块承载的 chunk 数**按 KB 差异很大**（`4a1dcb8b` 30/9≈3.33、`b9e74e82` 33/12≈2.75、`ea84fb72` 52/30≈1.73；最大 6 —— 2026-09-24 实测）—— 只要同一父块的多个 chunk 同时进入结果，就会输出逐字相同的正文。现状 `每文档 1 条` 恰好掩盖了这一点。
...
**父块标识的取法**：键为 `(doc_id, hash(parent_content))`；`parent_content` 缺失时退化为按该 chunk 自身保留（等价于不去重）—— 覆盖率按 KB 差异大（`4a1dcb8b` 30/38、`b9e74e82` 33/51、`ea84fb72` 仅 52/121）。
...
**代表分来自每条候选最终的 `.score`，由调用方在建 context 时填好**：`_dedup_by_parent` 的输入是 `list[RAGContext]`，按 `.score` 取每父块最高者；函数内部**不自取**分数（`RAGContext` 也没有 `distance` 字段）。分数语义**三态**：

| # | 情形 | `.score` 承载 | 是否落 `rerank done` |
|---|---|---|---|
| ① | 精排成功 | `relevance_score`（真实相关性分） | 是（`scored=rerank`） |
| ② | `rerank_results` 内部异常（`RERANK_FAILED`，`retrieval.py:148-161`） | `1 - distance` 回退分 | 是（`scored=fallback`） |
| ③ | 精排超时（`rag_tools.py` 的 `except TimeoutError`） | `1 - distance` 回退分 | 否（不产生该事件） |

按最终 `.score` 取值在三种情形下都自然正确；实现**不得**在 ②/③ 上另按"精排相关性分"的语义解释这些值，也**不得**让去重按"哪个先出现"兜底。
...
| **`dedup done`（新事件）** | `dropped` / `kept`（字段集仅此二者，**不含** `kb_id`/`query`，靠 trace_id 关联） | 天花板是否仍在生效（精排后 − 去重后） |

`dedup done` **由两条调用路径各自在去重后落**（`rerank_results` 内 / `rag_tools.py` 的降级分支），不放进 `_dedup_by_parent` 内部 —— 后者保持纯函数、不依赖日志设施，也避免为它引入 `kb_id` 参数（`rerank_results` 签名里没有 `kb_id`，加它会扩大改动面，与 D4「零契约影响」冲突）。
...
- **[`parent_content` 缺失的 chunk 不受约束]** → 覆盖率按 KB 差异大：`4a1dcb8b` 30/38≈79%、`b9e74e82` 33/51≈65%、`ea84fb72` **仅 52/121≈43%**（2026-09-24 实测）。缺失时退化为按自身保留，等价于不去重；结果是这部分 chunk 仍可能重复进入结果。**接受的代价**：宁可放过，不可错杀（错杀会丢召回）。⚠ 低覆盖率库（如 `ea84fb72`）去重效力被稀释，采样结论须把该因素与"单文档占满窗口"分开解释（见 Open Questions 3）。
...
3. **`ea84fb72`（121 chunk / 3 文档 / 单文档 85 chunk）未纳入先前的临时采样**：它是"单文档占满窗口"风险的最强样本，**必须**纳入正式采样（见 tasks 5.2）。⚠ 但该库 `parent_content` 覆盖率仅 52/121≈43%（三库最低），去重对它效力最弱 —— 采样结论须把"低覆盖率"与"单文档占满窗口"两个因素分开解释。
