...
系统 SHALL 支持用户在消息中显式调用 skill：消息以 `/` 开头，其后为 skill 名，格式 `/name 剩余文本`（name = skill 名，剩余文本 = 该次调用的任务文本）。后端 SHALL 解析该前缀并路由到对应 skill，**不依赖模型判断**。前端 SHALL 在新建对话页与历史对话页的输入框提供 `/` 命令补全。
...
- **WHEN** 用户发送 `/不存在的skill 问题`
- **THEN** 系统返回"skill 不存在 + 可用列表"，不当作普通文本静默处理

#### Scenario: 非命令前缀

- **WHEN** 消息不以 `/` 开头，或 `/` 后的名字不是已注册 skill
- **THEN** 按普通文本处理，不报错
