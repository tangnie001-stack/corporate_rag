...
- [ ] 5.6 新增 `src/cli/replay_trace.py`（**本期 L1**）：`--trace <id>` **扫全部 `app_*.log`（按天轮转，trace 可跨天；段位无关解析：trace 子串过滤 + 取最后一个 ` - ` 之后为 message，兼容新旧格式混存）** 解析该 trace 的 replay 事件（按 iteration）与行为信号 → 对当前 KB、当前配置重放检索打印 top 片段（source/page/score/片段头），**事件行"当时参数"并排对照、差异标注（drift）**；**无 `--max-per-doc`**（去重 N A/B 属离线实验，不在 CLI 范围）；输出标注"对当前 KB 重放（非历史快照）"；无检索轮 trace 给提示；单测覆盖日志解析与重放（检索 mock，不发真实网络），**解析器与 5.3 helper 编码 round-trip 配套**
