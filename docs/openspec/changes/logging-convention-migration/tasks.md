...
- [x] 2.1 在 `src/core/` 提供日志 helper：`log_event(prefix, event, **fields)`（或 `retrieval_signal(signal, query, iteration, **fields)`），统一 query 截断 40、k=v 拼装
- [x] 2.2 `retrieval_signal` 埋点 helper 单测（格式稳定、query 截断、trace_id 自动注入不重复）
...
- [ ] 4.3 检索行为信号端到端验证：跑一条真实 query，日志能定位到 reretrieve/to_web/cited 信号（配合 P1 Change 2）——待联调：依赖 retrieval-quality-signals（P1 Change 2）落地后，重启 app 加载新码后人工跑真实 query 验证
