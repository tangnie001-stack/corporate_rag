...
- **WHEN** 用户发送 `/financial-statement-analyzer 腾讯2024营收多少`
- **THEN** 系统解析出 skill=financial-statement-analyzer、task="腾讯2024营收多少"，加载并执行该 skill，跳过"模型是否委派"的判断
...
- **WHEN** 会话选定"财务专家"且用户使用 `/financial-statement-analyzer`
