...
- **WHEN** 生效预设声明 `skills: [financial-statement-analyzer]` 且当前为该会话首轮
...
- **WHEN** 用户消息为 `/financial-statement-analyzer 腾讯营收`
- **THEN** 注入 `financial-statement-analyzer` 的方法论（隐藏消息），本轮以该方法论执行
...
- **AND** 该判据经图状态（`AgentState` 字段）搬运，不改写主请求上下文
...
- **WHEN** 确认门判定"不通过"（用户拒绝 / 超时 / 澄清槽被占）
- **THEN** 直接出结论并标注"未经确认"，**不再进入 verify 重跑**（每轮最多重跑 1 次）
...
- **WHEN** 用户发送 `/financial-statement-analyzer 腾讯2024营收`
- **THEN** 会话历史中该条 user 消息内容为原文 `/financial-statement-analyzer 腾讯2024营收`（回放显示与用户输入一致）

#### Scenario: 组装 prompt 时剥离前缀

- **WHEN** 构建本轮 prompt（当前轮 query 与历史 user 消息）
- **THEN** user 消息内容为剥离前缀后的 `腾讯2024营收`，模型看不到 `/financial-statement-analyzer`
- **AND** 该 skill 的方法论仍以隐藏消息形式存在，持续生效

#### Scenario: 仅命令无剩余文本

- **WHEN** 用户只发送 `/financial-statement-analyzer`（无剩余文本）
