# observability-backend Specification

## Purpose
本项目自托管的 Langfuse 可观测后端（用于 LLM trace 采集）的部署与运维契约：以已 EOL 的 v2 线镜像自托管，复用应用既有的 PostgreSQL 实例与独立 database，不引入 ClickHouse / worker / 对象存储等 v2 专属或 v3 专属依赖，并在 dev 默认启用、可经 profile 关闭。
