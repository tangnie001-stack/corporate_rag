...
| F-15 | **BM25 检索每次调用全量重载索引**：`BM25Index.search`（`src/infra/search/bm25_index.py:86-90`）在**每次检索调用**都读文件并重建内存结构，无进程内缓存。2026-09-18 容器内实测（`b9e74e82…`，51 chunk / 内容 31881 字符 / pkl 236KB）：`pickle.load` = **58.2 ms**，`BM25Okapi` 从原文重建 = 5.6 ms，`get_scores` = 0.2 ms。`retrieve_kb` 单请求最多调用 5 次 → 单请求固定开销约 0.3 s，且随 chunk 数增长（按千文档 × 25–40 chunk/doc 估算，单次加载会到秒级）。这是**既有缺陷**，`bm25-index-durability` 不制造也不修（它改 JSON 后实测降到 5.6 ms 级，属顺带改善） | 加进程内缓存：以 `(kb_id, 文件 mtime)` 为键缓存已构建的 BM25 与 chunks，写/删索引时失效。备选：去掉落盘、按需从 **Chroma** 重建并缓存（见 `bm25-index-durability/design.md` 已评估并否决的理由；现状：dense 已迁 pgvector） | P2 | 中 | 无 |
...
| F-19 | **（P2 遗留）Chroma 残留清理**：P2 只删了 `chromadb` 的**代码路径**，`pyproject.toml` 的 `chromadb` 依赖、`data/chroma_persist` 数据目录、`deploy/chroma/` 仍在；`scripts/migrate_chroma_to_pg.py` / `scripts/dense_equivalence_check.py` / `scripts/clean_all_data.py` 仍用 Chroma 客户端与 `CHROMA_PERSIST_DIR` / `CHROMA_COLLECTION_PREFIX` | 规划进 P4「依赖与卷清理」。⚠ `data/chroma_persist` 是搬迁与等价性验收的唯一语料来源、也是回滚依据，**P4 前不得删** | postgres-storage-consolidation P2 遗留项 | P2 | 中 | postgres-storage-consolidation（P2 已完成） |
| F-20 | **（P2 遗留）死日志事件 `CHROMA_CLIENT_READY`**：`src/core/log_events.py:89` 的 Event 枚举项与 `src/core/log_event_specs.py:150-151` 的 EventSpec 已无发出点（`_warmup_chromadb` 随 P2 删除），但成对保留。⚠ 删除必须**两处同删** —— `log_events.py` 的 import 期 `_validate_registry` 会因多余 spec 抛 `AssertionError`，只删一处会让 import 直接崩 | 随 F-19 同批清理（或独立小变更）。不要只删枚举或只删注册表 | postgres-storage-consolidation P2 遗留项 | P2 | 低 | F-19 |
| F-21 | **（P2 遗留）死配置 `CHROMA_HOST` / `CHROMA_PORT`**：`src/config/settings.py:186-188` 定义后全仓零引用（含测试与脚本）。同文件的 `CHROMA_COLLECTION_PREFIX` / `CHROMA_PERSIST_DIR` **不是**死配置 —— 三个脚本仍依赖（见 F-19） | 待 F-19 迁移脚本后删除这两个键；`.env` / `.env.example` 若有同名键一并清 | postgres-storage-consolidation P2 遗留项 | P2 | 低 | F-19 |
| F-22 | **（P2 遗留，门禁盲区）`alembic/env.py` 未传 `compare_server_default=True`**：Alembic 该参数默认 `False` → autogenerate **根本不比较**任何表的 `server_default`，**所有表**的默认值漂移都不会被发现（全局盲区，不只 `chunks`）。P2 仅对 `chunks` 的 `source`/`page`/`metadata` 三列用只读断言旁路覆盖（Ruling P2-R11），`alembic/env.py` 本身未改 | 单独变更打开开关；打开后可能一次性暴露既有表的默认值漂移，需先评估影响面再决定修漂移还是调模型。**（P4 门禁项）**终审要求把「有归属 P4 的门禁项」显式登记，本条即该项（原 F-26 已并入本条） | postgres-storage-consolidation P2 遗留项 | P1 | 中 | 无 |
| F-23 | **（P2 遗留）`src/api/documents.py:246` 的 api→infra 越层调用**：`api/` 直接调 `svc.vector_store.delete_document(...)`（`AppService` 暴露的 `vector_store` 句柄），绕过 `services/`，违反 `CLAUDE.md`「api/ 不得直接调用 infra/」的规约。P2 只补了 `await`（正确性），未动越层（Ruling P2-R7）——属**既有**结构问题 | 把删除编排下沉到 `services/`（如 `DocumentService.delete_document`），api 只转调；涉及 service 层签名与既有测试，独立变更做 | postgres-storage-consolidation P2 遗留项 | P2 | 低 | 无 |
...
| F-25 | **（P4 覆盖，终审要求显式登记）双删除路径收编为一条**：API 端点 `src/api/documents.py:244-246` 走「先软删文档 → 再删分块」（旧序）——分块删除失败时文档已软删、分块成**永久孤儿**，且重试因 `ok=False` 不再删分块；而语义正确的 `DocumentService.delete_document`（**先删分块、抛错则不软删**）全仓**无 API 路由调用、是死代码**。P2 把分块删除从「吞异常」改为「传播」后，生产实际走的那条路的失败会暴露成「用户看到 500、文档却已删」。P4 的「入库/删除同事务」应同时收编两条为一条：API 端点改走 service，并去掉 `svc.document._doc_repo` 的私有属性直取（越层面见 F-23） | postgres-storage-consolidation P2 终审遗留（Ruling P2-R17） | P1 | 低 | postgres-storage-consolidation（P4） |
| F-27 | **（P4 覆盖，终审要求显式登记）存量 Chroma 字样清理（docstring 层）**：`src/chunking/validator.py:14`/`:21` 仍有「Chroma id」措辞（本轮修复波只改了 `src/infra/search/bm25_index.py`（P3 已删除）的 `rebuild_from_results` docstring）；随 F-19 的依赖/数据目录清理同批收口。`src/rag/retrieval.py:77` 的存量乱码属另一既有问题，不在此列 | postgres-storage-consolidation P2 终审遗留（Ruling P2-R17） | P2 | 低 | F-19 |
| F-28 | **（P3 遗留）`rank_bm25` 依赖未删**：P3 删除了 `bm25_index.py` 与全部装配链，但 `pyproject.toml` 的 `rank_bm25` 仍在（`scripts/lexical_probe.py` 的分组 A 仍用 `BM25Okapi` 做基线对照 —— 那是探针需要，不是生产依赖）。同批还有 `data/bm25_index` 目录（词法路的回滚依据，P3 刻意保留） | 随 F-19 的 P4 依赖与卷清理一并删；删依赖需要 `docker compose build --no-cache app`。来源：postgres-storage-consolidation P3 遗留项 | P3 | 低 | F-19 |
| F-29 | **（P3 遗留，spec 陈旧）`multi-query-retrieval` 的 requirement 正文引用已不存在的组件**：正文写「`retrieve_node` 并行遍历 `rewritten_queries` 列表逐条执行 dense + 词法两路检索」，但 harness 改造后检索是 `retrieve_kb` 工具 + 单条 query，`grep -rn "rewritten_queries\|retrieve_node" src/` **零命中**；`rrf_fusion_multi` 也随之全仓无调用方（仅测试）。该 change 的 delta 会把这句写进在效规格 | 由后续变更重写该 requirement（把"多查询合并检索"重述为与 `retrieve_kb` 工具链一致的形态），或与 `retrieval-fetch-and-dedup` 的重定基一并处理。P3 只做了命名同步（`dense + BM25 混合检索` → `dense + 词法两路检索`），**未改**结构性陈述。来源：2026-09-19 P3 执行期发现 | P2 | 中 | retrieval-fetch-and-dedup |
...
| F-31 | **测试污染真实 PG**：`tests/infra/db/test_db.py` 用 `user_id="test-user"` + 每次唯一名**直写真实 PostgreSQL**，**没有任何 teardown** → 每跑一次全量 pytest 就累积约 5 个知识库 / 6 个文档（P3 收尾实测 dev 库 `90|102|176|1`，而语料本应 `5|0|176|1`）。同一类问题还包括 `tests/infra/db/test_chunk_repo*.py` 等真实 PG 测试（它们**有** teardown，可作范式） | 给真实 PG 测试补 fixture 级 teardown（或统一走 `tests/reset_data.py` 的复位），并把"跑完门禁要复位"写进 `cookbook.md`。⚠ 影响面不止整洁：dev 界面上会看到大量 `test-user` 的垃圾知识库。来源：2026-09-19 P3 Task 11 验收期发现 | P2 | 低 | 无 |
