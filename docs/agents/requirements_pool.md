...
| F-17 | **（L3）prod `--workers 4` 与「生产单 worker」规则冲突**：`docker-compose.prod.yml` 用 `--workers 4`，而 `CLAUDE.md` 规定生产单 worker（流式生成状态在进程内，多 worker 会破）。4 × (pool_size 10 + max_overflow 10) ≈ **80 连接**，与同实例的 Langfuse 争抢 `max_connections`。属独立变更（涉及流式状态前提），也是托管化时 RDS 规格的输入约束 | postgres-storage-consolidation P1 遗留项 L3 | P1 | 低 | F-16（RDS 规格） |
