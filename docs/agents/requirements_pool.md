...
| F-26 | **（P4 覆盖，终审要求显式登记）`alembic/env.py` 补 `compare_server_default=True`**：Alembic 该参数默认 `False` → autogenerate **不比较任何表**的 `server_default`，是 P1 就有的**全局**盲区（P2 只对 `chunks` 的 `source`/`page`/`metadata` 三列做旁路断言，Ruling P2-R11）。直接打开会让 autogenerate 开始比较所有表、可能一次性暴露既有漂移，属**独立变更**；同一盲区已见 F-22，本条记录终审对「有归属 P4 门禁项」的要求 | postgres-storage-consolidation P2 终审遗留（Ruling P2-R17） | P1 | 中 | 无 |
