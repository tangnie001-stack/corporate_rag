...
| F-28 | **【已修，P4 Task 6】`rank_bm25` 依赖未删**：P3 删除了 `bm25_index.py` 与全部装配链，但 `pyproject.toml` 的 `rank_bm25` 仍在（`scripts/lexical_probe.py` 的分组 A 仍用 `BM25Okapi` 做基线对照 —— 那是探针需要，不是生产依赖）。同批还有 `data/bm25_index` 目录（词法路的回滚依据，P3 刻意保留） | **已修**（P4 Task 6，`8dd145d`）：`rank_bm25` 依赖已从 `pyproject.toml` 删除，镜像经 `docker compose build --no-cache app` 重建；探针脚本一并退役。`data/bm25_index` 目录仍在磁盘（Task 11 退役）。来源：postgres-storage-consolidation P3 遗留项 | P3 | 低 | F-19 |
