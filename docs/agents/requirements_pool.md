...
| F-27 | **（P4 覆盖，终审要求显式登记）存量 Chroma 字样清理（docstring 层）**：`src/chunking/validator.py:14`/`:21` 仍有「Chroma id」措辞（本轮修复波只改了 `src/infra/search/bm25_index.py` 的 `rebuild_from_results` docstring）；随 F-19 的依赖/数据目录清理同批收口。`src/rag/retrieval.py:77` 的存量乱码属另一既有问题，不在此列 | postgres-storage-consolidation P2 终审遗留（Ruling P2-R17） | P2 | 低 | F-19 |
...
| F-30 | **（P3 遗留）词法路质量的判据缺口**：词项命中探针（`docs/tmp/p3-lexical-probe-2026-09-19.md`）的判据是**集合成员**（"含该词项的 chunk 是否进 top-k"），而过滤臂的候选集恒 ≤ k ⇒ 该口径**结构上测不了排序质量**（C 组 `ts_rank`/`ts_rank_cd` 的零差是必然），未过滤的 BM25 臂又因池 ≫ k 与"过滤/未过滤"差异无法与分词配置归因。因此"生产形态 `jieba-tsrank` 在 A 组最低"这一实测事实**不可归因于分词质量**，"替换有收益/无收益"均未被本探针建立 | 需要 k ≪ 池规模的新判据，或等语料到位后的端到端 RAGAS 评估；在此之前**不得**把探针数字当质量基线。来源：postgres-storage-consolidation P3 遗留项（探针结论） | P2 | 中 | 无 |
