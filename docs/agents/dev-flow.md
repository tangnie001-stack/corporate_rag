...
跨子系统**且按判据二需要拆分**的变更，会同时用到 ⑤ 的两个产物，**不构成"一事两档"**。判据只有一条：**同一份文件里不出现两套可执行的步骤清单**。

- openspec change 只承载**为什么 + 规格**（`proposal.md` / `design.md` / `specs/`）
- `tasks.md` **不再承载任务清单**，退化为「**执行档索引**」：列出各执行档（计划文件，或"计划外动作"）与其完成标准，并指向落在 `docs/superpowers/plans/` 的计划文件
- 实施计划落在 `docs/superpowers/plans/`，由 ⑥ 执行（拆分时一份/阶段；不拆分时一份到底）
...
**只在「存在可独立结束的边界」时拆**，满足任一即拆：

| 判据 | 含义 | 怎么判断 |
|---|---|---|
| **A 执行器不同** | 某段工作的性质落在 ⑥「不适合走 SDD 的情形」里（纯部署 / 配置、含破坏性操作、无代码 diff），**必须换执行器** | 问"这段能切成 worktree 内可独立评审的代码任务吗？"——不能，就拆出去单独成档 |
| **B 可独立交付** | 存在一个交付点，此后可**独立合并或部署**，且后续阶段失败不推翻已交付部分的成立 | 问"**真的会在这里停或交接吗**？"——只是"能跑起来"不算 |
| **C 体量或接手** | 单份计划超出可读上限，或前后段由**不同会话 / 不同排期**接手 | —— |

**明确【不构成拆分理由】**（这几条曾被误当作理由用过）：

- **硬顺序依赖**（"底座先行"）—— 执行器（SDD / `executing-plans`）本身**串行**推进，先后在**单份计划内**用前置行即可表达；**拆文件不增加任何能力**
- **中途"能跑起来"** —— 见判据 B，"可运行" ≠ "可独立交付"
- **任务证据形式不同**（单测 / 实跑 / grep）—— 单份计划内每个任务各自写自己的验收证据即可
- **与在途 change 重叠** —— 这是**排序声明**，在计划顶部写一行就够，与拆不拆文件无关

> 口诀：**规模决定计划有多长；工作性质（谁执行）决定要不要拆阶段。**
> （"顺序依赖"不进这句 —— 它由执行器的串行性兜住。）

**实例**：`postgres-storage-consolidation` 跨了关系型存储 / 向量存储 / 词法检索 / 分块 / 取数融合 / 迁移 / 部署 / 依赖共八个面（proposal 自记"capability 面 14 个"）。P2 含数据搬迁、P3 含存量全量重写、P4 含**删卷**与 prod compose —— **后三段都命中判据 A**，且 P1 有独立收口（`0535c56`，命中 B）→ 拆 P1–P4，`tasks.md` 退化为索引。

**反例一**：`langfuse-v2-downgrade` 有 60 条任务，却只跨"可观测后端部署"一个工作流，`src/` 零改动 → 按判据 A 它甚至**不该走 SDD**（纯部署 / 配置，见 ⑥）；不拆阶段是对的。

**反例二（旧判据的错例，2026-09-22 修正）**：`langfuse-trace-wiring` 跨 6 个模块、有硬顺序依赖（清理必须在接线验证之后，否则删掉的正是唯一可用的观测依据），**按旧的四条会被拆成 3 份阶段计划** —— 但五段工作全是普通代码任务、执行器相同。按本判据只有「定时清理接入 + 真删验证」命中判据 A（部署 + 破坏性数据操作），故正确地拆为 **1 份实施计划 + 1 个计划外动作**。
