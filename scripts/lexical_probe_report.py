...
def render_report(
    *,
    ...
    caveat = (
        f"> {total} 分块属**小语料**，k={k} 下命中集合规模有限，多数词项会"
        "**平凡通过**；探针的用途是在同一语料上横向比配置，"
        "**既不测排序质量也不测语义召回**。"
    )
    term_source = (
    ...
    term_filter = (
        f"- **词项筛选**：`{df_min} <= df <= {df_ratio}×N`，"
        "剔除 df=1 的无从判断项与高频的平凡通过项；再按 `(df 升序, 词项)` "
        "取最稀有的前若干项。**该取舍使词项偏向最难召回的一端，各臂命中率因此是"
        "偏保守（偏病态）的下界，不得读作平均检索质量。**"
    )
    kb_pin = (
        "- **单库钉定**：每个词项被钉在「按 kb_id 排序、第一个含该词项的知识库」上，"
        "只在该单库内检索。多库语料下这会进一步压低命中率 —— 同样属于"
        "**使结果更病态**的取舍。"
    )
    rank_inert = (
        f"- **排序近乎不起作用**：本语料下探针词项全局 df 均落在下界 df={df_min}，"
        f"在钉定的单个库内只有极少数行含该词项，远小于 k={k}。因此「是否命中」"
        "这一集合成员判据对排序不敏感；排序差异需候选集远大于 k 才能测出。"
    )
    ...
    a1_intro = (
        "两臂除**分词方式**外完全同口径：同 BM25Okapi、同词项集、同在每库全池上"
        "排序（不过滤候选）、同 k。"
    )
    a1_measured = (
    ...
    a1_rank_note = (
        "- ⚠️ 由于排序近乎不起作用（见口径说明），该差值主要反映**候选集合成员**，"
        "而非排序质量。"
    )
    ...
    b_substring = (
        ...
        "该词项的行 —— 命中是 WHERE 子句的同义反复。该行还是一次**无索引全表扫描且"
        "不排序**（`ORDER BY doc_id, chunk_index`），**不是可选策略，不得读作"
        "「LIKE 最好」**。"
    )
    b_substring_vars = (
    ...
    c_wording = (
        "- ⚠️ 零差不表示两个打分函数「同分」，而是：**候选集相同且规模 ≤ k 时，"
        "该口径对排序不敏感**，函数差异**在此口径下无法测出**（见口径说明）。"
        "要比较打分质量，需让候选集远大于 k 并比较 top-k 排序，本探针未做。"
    )
